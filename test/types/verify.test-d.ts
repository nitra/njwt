...
import { Jwt, Verifier, verify, createVerifier } from "@nitra/njwt";
...
expectType<void>(
  verifier.setKeyResolver(function (
    keyId: string,
    callback: (err: Error | null, resolvedKey: string) => Jwt | undefined
  ) {
...
expectType<Verifier>(
  verifier.withKeyResolver(function (
    keyId: string,
    callback: (err: Error | null, resolvedKey: string) => Jwt | undefined
  ) {
    return callback(null, "resolvedKey");
...
expectError<Verifier>(
  verifier.withKeyResolver(function (
    keyId: string,
    callback: (err: string | null, resolvedKey: string) => Jwt | undefined
  ) {
    return callback(null, "resolvedKey");
  })
);
...
expectType<void>(
  verifier.verify("tokenString", function (err: Error | null, token: Jwt) {
    console.log(token);
  })
);
